...
_ACTION_PATTERNS: list[tuple[re.Pattern[str], str]] = [
    ...
    (re.compile(r"^\s*open\s+email\b", re.I), "Open email"),
    (re.compile(r"^\s*open\s+message\b", re.I), "Open message"),
    (re.compile(r"^\s*open\s+account\b", re.I), "Open account"),
    (re.compile(r"^\s*update\s+status\b", re.I), "Update status"),
