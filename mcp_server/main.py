...
def get_procedural_context_pack(
    ...
    """Return one hard-capped observational workflow-memory bundle for an agent.

    The bundle is derived evidence, not organizational policy or an instruction to
    act. It does not auto-inject context or execute any workflow step.
    """
    name = "get_procedural_context_pack"; _begin(name)
    return _finish(name, _get("/v1/procedural-memory/context-pack", {
        "family_key": family_key,
        "current_steps": current_steps,
        "after_step": after_step,
        "min_support": min(max(2, int(min_support)), 100),
