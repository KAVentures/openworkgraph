...
def _browser_payload() -> dict:
    return {
        "event_id": "v48-browser-event",
        "observed_at": "2026-09-18T16:00:01+00:00",
        "sensor_id": "browser:test",
        "sensor_version": "1.9.0-v48-local-auth",
        ...
        "target": {"tag": "button", "role": "button", "label": "Send"},
        "metadata": {},
    }
...
def test_dashboard_bootstrap_uses_cookie_not_master_token(secured_api):
    ...
    with httpx.Client(base_url=base) as client:
        unauth = client.get("/v1/summary?scope=current")
        ...
        summary = client.get("/v1/summary?scope=current")
        assert summary.status_code == 200
        config = client.get("/v1/mcp-connection-config")
        assert config.status_code == 200
        assert config.json()["token"] == secured_api["mcp_token"]


def test_browser_requires_server_proof_and_signed_request(secured_api):
    base = secured_api["base"]
    origin = "chrome-extension://openworkgraphtest"

    preflight = httpx.options(
        f"{base}/v1/browser-events",
        headers={
            "Origin": origin,
    ...
    challenge = httpx.post(
        f"{base}/v1/browser-challenge",
        json={"nonce": challenge_nonce},
        headers={"Origin": origin},
    )
    assert challenge.status_code == 200
    expected = hmac.new(
        secured_api["browser_secret"].encode("utf-8"),
        f"openworkgraph-server-proof:{challenge_nonce}".encode("utf-8"),
        hashlib.sha256,
    ).hexdigest()
    ...
    accepted = httpx.post(
        f"{base}/v1/browser-events",
        content=body,
        headers={"Origin": origin, "Content-Type": "application/json", "Authorization": auth},
    )
    assert accepted.status_code == 200
    assert accepted.json()["status"] == "ok"

    # Replaying the exact HMAC nonce is rejected even if the body is unchanged.
    replay = httpx.post(
        f"{base}/v1/browser-events",
        content=body,
        headers={"Origin": origin, "Content-Type": "application/json", "Authorization": auth},
...
def test_mcp_http_endpoint_rejects_unauthenticated_clients(secured_api):
    mcp_port = _free_port()
    env = dict(secured_api["env"])
    env["WORKFLOW_OBSERVER_API"] = secured_api["base"]
    process = subprocess.Popen(
        [sys.executable, "-m", "uvicorn", "mcp_server.http_app:app", "--host", "127.0.0.1", "--port", str(mcp_port)],
        cwd=ROOT,
        env=env,
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
    ...
    try:
        ...
        payload = {
            ...
            "params": {
                ...
                "capabilities": {},
                "clientInfo": {"name": "v48-test", "version": "1"},
            },
        }
        ...
        assert httpx.post(endpoint, json=payload, headers=wrong, timeout=5).status_code == 401
        good = dict(headers, Authorization=f"Bearer {secured_api['mcp_token']}")
        response = httpx.post(endpoint, json=payload, headers=good, timeout=5)
...
def test_browser_pairing_bundle_is_runtime_only_and_launcher_preserves_core_paths():
    ...
    assert "mcp_server.http_app:app" in start
...
def test_local_secret_permissions_are_private_on_posix(tmp_path):
    if os.name == "nt":
        pytest.skip("POSIX mode bits are not the Windows ACL model")
    auth_dir = tmp_path / "auth"
    ensure_api_token(directory=auth_dir)
    mode = (auth_dir / ".api_token").stat().st_mode & 0o777
    assert mode == 0o600
