...
def create_app(*, settings: GatewaySettings | None = None, db: GatewayDB | None = None) -> FastAPI:
    ...
    def health() -> dict[str, Any]:
        return {"status": "ok", "server": "OpenWorkGraph Gateway", "version": "0.53.0", "storage": "postgresql" if db.is_postgres else "sqlite-development", "self_hosted": True}

    @app.get("/v1/capabilities")
    def capabilities() -> dict[str, Any]:
        return {"canonical_data_layer": "privacy_hardened_raw_rich_evidence", "evidence_contract": dict(RAW_RICH_EVIDENCE_CONTRACT), "interfaces": ["REST", "MCP-adapter"], "cloud_account_required": False, "inferred_tasks_authoritative": False}
    ...
    def enroll_device(request: EnrollmentRequest, authorization: str | None = Header(default=None)) -> dict[str, Any]:
        ...
        db.put_token(token_id=token_id, token=token, token_type="device", organization_id=organization_id, actor_id=request.actor_id.strip(), device_id=device_id, scopes=set(DEVICE_SCOPES), replace_device=True)
        db.audit(organization_id=organization_id, principal_id=token_id, action="device.enrolled", details={"device_id": device_id})
        return {"token": token, "token_id": token_id, "organization_id": organization_id, "actor_id": request.actor_id.strip(), "device_id": device_id, "scopes": sorted(DEVICE_SCOPES), "note": "The device token is shown once. Store it in the endpoint credential store/file."}
    ...
    def create_integration_token(request: IntegrationTokenRequest, authorization: str | None = Header(default=None)) -> dict[str, Any]:
        ...
        db.audit(organization_id=organization_id, principal_id="gateway-admin", action="integration.token.created", details={"token_id": token_id, "scopes": sorted(scopes), "label": request.label[:120]})
        return {"token": token, "token_id": token_id, "organization_id": organization_id, "scopes": sorted(scopes), "note": "The service token is shown once. Store it in the integration secret store."}
    ...
    def device_policy(p: Principal = Depends(principal)) -> dict[str, Any]:
        ...
        return {"organization_id": p.organization_id, "policy": db.get_policy(p.organization_id), "policy_is_ceiling": True, "note": "Endpoint local policy can only further restrict what is synchronized."}
    ...
    def ingest(batch: EvidenceBatch, p: Principal = Depends(principal)) -> dict[str, Any]:
        ...
        db.audit(organization_id=p.organization_id, principal_id=p.token_id, action="evidence.batch.ingested", details={"received": len(safe_events), "inserted": inserted})
        return {"inserted": inserted, "acknowledged_event_ids": acknowledged}

    @app.get("/v1/workflow-trace")
    def trace(since: str | None = None, until: str | None = None, cursor: str | None = None, limit: int = Query(default=100, ge=1, le=500), query: str | None = None, actor_id: str | None = None, device_id: str | None = None, session_id: str | None = None, event_type: str | None = None, p: Principal = Depends(principal)) -> dict[str, Any]:
        require(p, "evidence:read")
        result = workflow_trace(db, organization_id=p.organization_id, since=since, until=until, cursor=cursor, limit=limit, query=query, actor_id=actor_id, device_id=device_id, session_id=session_id, event_type=event_type)
        db.audit(organization_id=p.organization_id, principal_id=p.token_id, action="evidence.trace.read", details={"returned": result["returned"]})
        return result

    @app.post("/v1/search")
    def search(request: SearchRequest, p: Principal = Depends(principal)) -> dict[str, Any]:
        require(p, "evidence:read")
        result = workflow_trace(db, organization_id=p.organization_id, since=request.since, until=request.until, limit=request.limit, query=request.query, actor_id=request.actor_id, device_id=request.device_id, session_id=request.session_id)
        result["query"] = request.query
        return result

    @app.get("/v1/context/current")
    def current_context(actor_id: str | None = None, device_id: str | None = None, limit: int = Query(default=50, ge=1, le=200), p: Principal = Depends(principal)) -> dict[str, Any]:
        ...
        return {"rows": rows, "returned": len(rows), "data_layer": "privacy_hardened_raw_rich_evidence", "evidence_contract": dict(RAW_RICH_EVIDENCE_CONTRACT), "interpretation": "Recent observed evidence only. The Gateway does not assert a task name, workflow family, productivity score, or inferred intent."}

    @app.get("/v1/transfers")
    def transfers(since: str | None = None, until: str | None = None, actor_id: str | None = None, limit: int = Query(default=300, ge=1, le=1000), p: Principal = Depends(principal)) -> dict[str, Any]:
        require(p, "transfers:read")
        result = workflow_trace(db, organization_id=p.organization_id, since=since, until=until, actor_id=actor_id, limit=min(limit, 500))
        ...
        items = [{"clipboard_transfer_id": key, "events": value, "event_count": len(value)} for key, value in grouped.items()]
        return {"items": items, "returned": len(items), "data_layer": "privacy_hardened_raw_rich_evidence", "clipboard_contents_captured": False}
