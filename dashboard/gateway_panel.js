(() => {
  ...
  function statusHtml(s) {
    ...
    return `<div style="display:flex;justify-content:space-between;gap:14px;align-items:flex-start;flex-wrap:wrap"><div><h3 style="margin:0 0 6px">Organization Gateway</h3><div><strong>${e(modeLabel(s))}</strong> · ${url}</div><div class="muted" style="margin-top:5px">${e(last)}${s.worker_running?' · sync worker running':''}</div></div><div class="modal-actions">${controls}</div></div>${err}${quarantine}<div class="note" style="margin-top:10px">Local recording always continues independently. New connections share evidence captured <strong>after enrollment</strong>; earlier local history stays local. While sharing is paused, evidence still records locally but that paused interval is <strong>never backfilled</strong> on resume. No OpenWorkGraph cloud account is required.</div>`;
  }
  ...
  window.openGatewaySetup = function() {
    ...
    const content=`<p>Connect this endpoint to a <strong>customer-controlled OpenWorkGraph Gateway</strong>. Prefer a short-lived, single-use enrollment code issued by the Gateway administrator. Existing v0.53 administrator enrollment secrets remain temporarily compatible.</p><div class="note" style="margin-bottom:12px"><strong>Shared by default after you connect:</strong> privacy-hardened app/window/page context, timestamps and effort/interaction metadata, safe UI/semantic labels, and copy/cut/paste occurrence/linkage. <strong>Pre-enrollment history stays local.</strong> Excluded/private evidence stays local.<br><br><strong>Never sent by this Gateway path:</strong> typed text, clipboard contents, ordinary key identities, password values, or screenshot bytes. Organization policy may further reduce what is shared; it cannot broaden your local sharing policy.</div><label>Gateway URL</label><input id="gwUrl" style="width:100%;box-sizing:border-box;margin:5px 0 10px" placeholder="https://openworkgraph.company.internal"><label>Organization ID</label><input id="gwOrg" style="width:100%;box-sizing:border-box;margin:5px 0 10px" placeholder="acme"><label>Actor ID <span class="muted">(optional; normally bound by the enrollment code)</span></label><input id="gwActor" style="width:100%;box-sizing:border-box;margin:5px 0 10px" placeholder="employee or pseudonymous ID"><label>Enrollment code</label><input id="gwToken" type="password" autocomplete="off" style="width:100%;box-sizing:border-box;margin:5px 0 10px" placeholder="Single-use enrollment code"><div class="note">Non-local Gateways require HTTPS. Nothing is shared until enrollment succeeds. Pausing later keeps that paused interval permanently local while local capture continues.</div><div class="modal-actions" style="margin-top:14px"><button id="gwConnectBtn">Connect and share from now</button><button class="secondary" onclick="closeModal()">Cancel</button></div>`;
